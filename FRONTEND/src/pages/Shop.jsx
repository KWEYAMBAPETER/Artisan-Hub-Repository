  ...
  return (
    <>
      ...
      <Container size="lg" py="lg" className="space-y-6">
        ...
        <SimpleGrid cols={{ base: 1, sm: 2, md: 3 }} spacing="xl" verticalSpacing="lg">
          {paginated.map(({ id, attributes }) => {
            const imgUrl = `${BACKEND_URL}${attributes.images?.data[0]?.attributes?.url}`;

            return (
              <Card shadow="sm" radius="md" withBorder key={id} className="h-[320px] flex flex-col justify-between">
                <Card.Section>
                  <Image
                    src={imgUrl}
                    h={160}
                ...
                <Group justify="space-between" mt="sm" wrap="nowrap">
                  <Text fw={600} size="md" truncate>{attributes.title}</Text>
                  <Badge color="#D97706" variant="light" size="sm">{attributes.category}</Badge>
                </Group>

                <Text fw={700} size="md" mt="xs" mb="xs" className="text-amber-700">
